...
def master_callbacks(call):
    ...
    if action == 'add_bot':
        ...
        markup.add(InlineKeyboardButton("🔙 Back to Menu", callback_data="m_main_menu"))
        master_bot.edit_message_text("🎬 *Select a Product to add Permanent Demo Video:*", chat_id, call.message.message_id, reply_markup=markup)

    elif action == 'wel_pc':
        msg = master_bot.send_message(chat_id, "🖼️ *Send the Welcome Photo with your desired long caption now:*")
        master_bot.register_next_step_handler(msg, save_welcome_photo_caption)
        ...
        msg = master_bot.send_message(chat_id, "🖼️ *Send the QR Code Photo now:*")
...
def select_product_for_video(call):
    ...
    admin_video_states[call.message.chat.id] = cat_id
    msg = master_bot.send_message(call.message.chat.id, "🎬 *Now send the Demo Video (or Video File ID) to save permanently:*")
    master_bot.register_next_step_handler(msg, save_category_video_from_button)
    master_bot.answer_callback_query(call.id)

def save_category_video_from_button(message):
    chat_id = message.chat.id
    cat_id = admin_video_states.get(chat_id)
    if not cat_id:
        master_bot.send_message(chat_id, "❌ Session expired. Please select product again from menu.")
        return
    ...
    if not video_id:
        master_bot.send_message(chat_id, "❌ Please send a valid video or file ID.")
        return

    try:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO category_videos (category_id, video_file_id) VALUES (?, ?)", (cat_id, video_id))
        conn.commit()
        cursor.close()
        conn.close()
        master_bot.send_message(chat_id, f"✅ *Permanent Demo video successfully saved to Product ID {cat_id}!*")
    except Exception as e:
        master_bot.send_message(chat_id, f"❌ Error saving video: `{e}`")

def save_welcome_photo_caption(message):
    chat_id = message.chat.id
    if not message.photo:
        master_bot.send_message(chat_id, "❌ *Please send a valid photo with caption!*")
        return
    
    photo_file_id = message.photo[-1].file_id
    caption_text = message.caption if message.caption else "✨ *Welcome to Store* ✨"
    ...
    master_bot.send_message(chat_id, "✅ *Welcome Photo and Caption successfully updated for all client bots!*")
...
def save_qr_photo(message):
    ...
    cursor.close()
    conn.close()
    master_bot.send_message(message.chat.id, f"✅ *Payment QR Code successfully saved!*")
...
def save_support_link(message):
    ...
    cursor.close()
    conn.close()
    master_bot.send_message(message.chat.id, f"✅ *Support / DM Link successfully updated to:* `{link}`")
...
def run_client_bot(token):
    ...
    def handle_category_click(call):
        ...
        # फालतू प्रोडक्ट डिस्क्रिप्शन मैसेज पूरी तरह हटा दिया गया है
        ...
        if videos:
            ...
            for v in videos:
                try:
                    ...
                    logger.error(f"Failed to send individual video: {e}")
    ...
    def receive_screenshot(message, order_id):
        ...
        username_str = f"@{user.username}" if user.username else "No Username (Direct ID)"
        ...
        if SUPER_ADMIN_ID:
            admin_txt = f"🔔 *NEW PAYMENT PROOF & BILL DETAIL!*\n\n" \
                        f"🆔 Order ID: `#{order_id}`\n" \
...
def handle_order_approval(call):
    ...
    if action == 'app':
        ...
        try:
            success_msg = f"🎉 *Payment Approved!*\n\nYour order `#{order_id}` for *{order['name']}* has been verified successfully."
            if chan and chan['invite_link']:
                success_msg += f"\n\n🔗 *Your Premium Link / Channel:* {chan['invite_link']}"
            else:
                success_msg += f"\n\n🔗 *Your Premium Link:* https://t.me/+YourChannelInviteLink"
        ...
        try:
            master_bot.send_message(user_chat_id, f"❌ *Payment Rejected!*\n\nYour payment proof for order `#{order_id}` was invalid or unclear. Please contact support.")
        except Exception:
            pass

print("Initializing execution directly...", flush=True)
init_db()
...
load_all_bots()
print("Master Control Bot is running with SQLite... Starting polling...", flush=True)
