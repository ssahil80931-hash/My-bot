...
def run_client_bot(token):
    ...
    def client_start(message):
        ...
        for cat in categories:
            # यहाँ बटन के साथ स्टाइल जोड़ा गया है ताकि इंटरफ़ेस और प्रीमियम दिखे
            btn = InlineKeyboardButton(f"🛒 {cat['name']} - ₹{cat['price']}", callback_data=f"cat_{cat['id']}")
            markup.add(btn)
    ...
    def handle_category_click(call):
        ...
        if videos:
            ...
            client_bot.send_message(chat_id, "ℹ️ *No demo videos uploaded for this category yet.*")
